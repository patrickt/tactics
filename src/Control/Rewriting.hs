...
recursively ::
  (Alternative m, Monad m, Corecursive t, Recursive t, Foldable (Base t)) =>
  Rewrite t a ->
  t ->
  m a
...
paraRewrite ::
  (Alternative m, Monad m, Corecursive t, Foldable (Base t)) =>
  Rewrite t a ->
...
applyAll ::
  (Corecursive t, Recursive t, Traversable (Base t)) =>
  Rule t ->
  Rule t
